...
class openSooq(ShopBase):
    ...
    def get_item_from_dev(self, search_item: Tag) -> Item:
        title_elem = search_item.find('h2', class_='font-20 breakWord overflowHidden')
        title = title_elem.text.strip() if title_elem else 'N/A'

        price_elem = search_item.find('div', class_='postPrice ms-auto bold alignSelfCenter font-19')
        ...
        link_element = search_item.find('a', class_='flex flexNoWrap p-16 blackColor radius-8 grayHoverBg ripple boxShadow2 relative')
...
if __name__ == "__main__":
    open_sooq = openSooq()
    data = open_sooq.get_items("خزانة")
